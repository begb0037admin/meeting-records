...
function addItem(data = {}) {
  ...
  const assistantBodyId = `assistant-body-${++assistantBodyCounter}`; const assistantToggle = $(".assistant-toggle", el); const assistantBody = $(".assistant-body", el); assistantBody.id = assistantBodyId; assistantToggle.setAttribute("aria-controls", assistantBodyId); assistantToggle.addEventListener("click", () => { const expanded = assistantToggle.getAttribute("aria-expanded") === "true"; assistantToggle.setAttribute("aria-expanded", String(!expanded)); assistantBody.hidden = expanded; }); const extractBodyId = `extract-body-${++assistantBodyCounter}`; const extractToggle = $(".extract-toggle", el); const extractBody = $(".extract-body", el); extractBody.id = extractBodyId; extractToggle.setAttribute("aria-controls", extractBodyId); extractToggle.addEventListener("click", () => { const expanded = extractToggle.getAttribute("aria-expanded") === "true"; extractToggle.setAttribute("aria-expanded", String(!expanded)); extractBody.hidden = expanded; });
