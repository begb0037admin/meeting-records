...
async function copyConversation(el) {
  const btn = $(".copy-chat", el);
  const text = [...$(".chat-messages", el).querySelectorAll(".chat-line")].map((line) => line.textContent).join("\n\n");
  const original = btn.innerHTML;
  const flash = (label) => { btn.textContent = label; setTimeout(() => { btn.innerHTML = original; }, 1500); };
  if (!text) return flash("Nothing to copy yet");
  try {
    if (navigator.clipboard?.writeText) await navigator.clipboard.writeText(text);
    else { const ta = document.createElement("textarea"); ta.value = text; ta.style.position = "fixed"; ta.style.opacity = "0"; document.body.append(ta); ta.focus(); ta.select(); document.execCommand("copy"); ta.remove(); }
    flash("Copied!");
...
function addItem(data = {}) {
  ...
  $(".chat-send", el).onclick = () => sendChat(el); $(".chat-input", el).addEventListener("keydown", (e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendChat(el); } }); $(".mic", el).onclick = () => recordVoice(el); $(".listen", el).onclick = () => listen(el); $(".copy-chat", el).onclick = () => copyConversation(el);
