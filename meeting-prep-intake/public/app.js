...
// Plain-English label for a carried-forward item. The underlying pointer
// (fromIntake file path + fromItemId) stays in el.dataset.carry / the
// submitted payload for tracking — it's just never shown to Kevin, who only
// needs to know when the item last came up and to make a disposition call.
const MONTH_ABBR = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
function carryForwardLabel(carryForward) {
  const match = /(\d{4})-(\d{2})-(\d{2})\.json$/.exec(carryForward?.fromIntake || "");
  const when = match ? `${Number(match[3])} ${MONTH_ABBR[Number(match[2]) - 1]}` : null;
  return when ? `Carried over from your last meeting (${when}). Has this been resolved?` : "Carried over from your last meeting. Has this been resolved?";
}
