...
**Phase 5** (merged, live): `POST /api/intakes/pending` reads the latest eligible short-lived roadmap draft from `CHAT_KV` (skipping any date that already has a locked, submitted intake); `POST /api/intakes/pending/write` accepts those drafts only from the secret-gated desktop automation (`X-Automation-Secret` header checked against the `PENDING_WRITE_SECRET` Worker secret). `automation/extract_hr_roadmap_pending.py` reads the local `HR Systems Roadmap MASTER.xlsm` "Work Tracker" sheet weekly (Windows Task Scheduler, Thursdays), selects active items due for review by the next Friday's meeting, and posts an editable draft for `hr-systems-roadmap`. Run it with `--dry-run` to print the computed payload without a secret or HTTP request. When Kevin selects "HR Systems Roadmap" in the browser, any pending draft is pre-filled into the form automatically, tagged "Pre-populated from ... — review before submitting." Pending drafts are never auto-submitted and never write a GitHub intake; only a locked `/api/intakes/submit` call ever does that.
